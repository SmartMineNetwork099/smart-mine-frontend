...
import SingleUserData from "@/modules/userBinary/levelTree/SingleUserData";
...
import { getuserBinaryTree } from "@/apis/userBinaryApis";
  ...
  return (
    <>
      <div className={`flex flex-col items-center ${disabled ? 'cursor-default' : 'cursor-pointer'}`} >
        ...
        <p className={`mt-1 text-xs sm:text-sm font-mono font-semibold ${disabled ? 'text-gray-400' : 'text-cyan-400'}`}> {child?.userId
  ? `...${child.userId.toString().slice(-5)}`
  : "----------"}</p>
        <Button
...
const UserBinaryTree = () => {
  ...
  const payload = searchId
    ? { userId: searchId }
    : {  walletAddress };
    const data = await getuserBinaryTree();
    console.log(data, 'binarytreedataaaaaaa123321')
